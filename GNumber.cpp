#include "GNumber.h"
GNumber::GNumber():Drawable(),m_Value(0),m_nbDigit(4),v_VertexBuffer(NULL),m_nbVertices(0),v_IndexBuffer(NULL),m_Faces(0),
                v_NormalsBuffer(NULL),m_nbNormals(0),v_TexCoords(NULL),m_nbTexCoords(0),m_Material(NULL),m_Scale(1.0f){
    GenerateMesh();
};
GNumber::~GNumber(){this->Destroy();};
void GNumber::Destroy(){
    if(v_VertexBuffer){
        free(v_VertexBuffer);
        v_VertexBuffer=NULL;
        m_nbVertices=0;
...
void GNumber::AddValue(unsigned int  Value){
...
void GNumber::setValue(unsigned int Value){
    if(Value<=9999)
        m_Value=Value ;
    else
        m_Value=9999 ;
...
unsigned int GNumber::getValue(){return m_Value;};
void GNumber::setScale(float value){
    m_Scale=value ;
};
/*void GNumber::setPosition(Vertex2d Pos){
    m_Pos.x=Pos.u;
    m_Pos.y=Pos.v;
    m_Pos.z=0.0f;
};
int GNumber::setOrientation(Vertex2d Dir){
    m_Dir.x=Dir.u;
    m_Dir.y=Dir.v;
    m_Dir.z=0.0f;
...
void GNumber::Draw(float* ViewMtx){
bool verif=false ;
...
int GNumber::GenerateMesh(){
    ...
    m_Material->TextureMap->Pixels=NULL ;
    lodepng_decode32_file(&(m_Material->TextureMap->Pixels),
                          &(m_Material->TextureMap->Width),
                          &(m_Material->TextureMap->Height),
                          "Data//numbers.png");
    ///////////////////////
    glGenTextures(1, &(m_Material->TextureId));
	glBindTexture(GL_TEXTURE_2D, m_Material->TextureId);
	glTexImage2D(GL_TEXTURE_2D,
				 0,
				 GL_RGBA,
				 m_Material->TextureMap->Width, m_Material->TextureMap->Height,
...
void GNumber::UpdateMtx(){
    Drawable::UpdateMtx();
    m_TransMtx[0]*=m_Scale;
    m_TransMtx[5]*=m_Scale;
    m_TransMtx[10]*=m_Scale;
};
void GNumber::setNBDigits(unsigned int Nb){
    if(Nb>0 && Nb<5){
        m_nbDigit=Nb;
        RegenTextureCoord();
    }
